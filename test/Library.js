...
describe("Library Contract",function(){
 ...
 describe("Get Book",function(){
   ...
   it("Should return the correct finished books",async function(){
    let booksFromChain=await library.getUnFinishedBooks();
  ...
  describe("Set Finished",function(){
    ...
    const BOOK_ID=0;
    const BOOK_FINSIHED=true;

    await expect(library.SetFinished(BOOK_ID,BOOK_FINSIHED)).to.emit(library,"SetFinished").withArgs(BOOK_ID,BOOK_FINSIHED);
  });
});
